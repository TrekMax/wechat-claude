...
wechat-claude 是一个桥接服务，通过 iLink 协议将微信消息连接到 Claude API。它接收微信用户的消息，转发给 Claude 处理，并将 Claude 的回复发送回微信。
...
│   ├── bridge.ts               # 核心编排器
│   ├── adapter/
│   │   ├── inbound.ts          # 微信 → Claude 内容转换
│   │   └── outbound.ts         # Claude → 微信文本格式化
│   ├── claude/
...
| 包名 | 用途 |
|------|------|
| `@xmccln/wechat-ilink-sdk` | 微信 iLink 协议（认证、消息收发、媒体加解密） |
| `@anthropic-ai/sdk` | Claude API 客户端 |
| `qrcode-terminal` | 终端二维码渲染（用于扫码登录） |
