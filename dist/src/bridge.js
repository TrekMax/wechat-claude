import { WeixinSDK, TokenAuthProvider, MediaDownloader, ApiClient, ApiEndpoints, } from "@xmccln/wechat-ilink-sdk";
...
export class WeChatClaudeBridge {
    ...
    constructor(config, token) {
        ...
        if (config.mode === "api") {
            ...
            this.acpSessions = new AcpSessionManager({
                ...
                onProgress: (userId, contextToken, text) => this.sendProgressText(userId, contextToken, text),
                onImageReceived: (userId, contextToken, data, mimeType) => this.sendImage(userId, contextToken, data, mimeType),
                onFileReceived: (userId, contextToken, data, fileName, mimeType) => this.sendFileBuffer(userId, contextToken, data, fileName, mimeType),
                sendTyping: (userId, contextToken) => this.sendTypingIndicator(userId, contextToken),
            });
        }
        this.sdk.onMessage((msg) => {
            if (this.debug) {
    ...
    async start() {
        ...
        log("bridge", `Started in ${this.config.mode.toUpperCase()} mode, listening for WeChat messages`);
    }
    destroy() {
     ...
     */
    async handleCommand(userId, contextToken, text) {
        ...
        if (cmd === "/debug") {
            ...
            await this.reply(userId, contextToken, this.debug
        ...
        if (cmd === "/model") {
            ...
            if (this.config.mode === "acp") {
                if (isViewCmd) {
                    ...
                    await this.reply(userId, contextToken, `当前模型 / Current model: ${current}\n\n切换方式 / Switch:\n/model sonnet (均衡)\n/model haiku (最快)\n/model opus (最强)`);
                }
                else if (validModels.some(m => m === arg || m.includes(arg))) {
                    this.acpSessions.setModel(userId, arg);
                    await this.reply(userId, contextToken, `模型已切换为 ${arg}\n会话将在下条消息时重启。\n\nModel switched to: ${arg}`);
                }
                else {
                    await this.reply(userId, contextToken, `未知模型: ${arg}\n可用选项 / Available: sonnet, haiku, opus`);
                }
            }
            else {
                await this.reply(userId, contextToken, `当前模型 / Current model: ${this.config.claude.model}\n(API 模式，需重启更换 --model)`);
            }
            return true;
        }
        ...
        if (cmd === "/new" || cmd === "/reset" || cmd === "/clear") {
            if (this.config.mode === "acp") {
                ...
                await this.reply(userId, contextToken, "会话已重置，下条消息将启动新的智能体。\nSession reset. Next message starts a new agent.");
            }
            else {
                this.sessions.resetSession(userId);
                await this.reply(userId, contextToken, "对话历史已清空。\nConversation has been reset.");
            }
            return true;
        }
        ...
        if (this.config.mode === "acp") {
            if (cmd === "/show-thoughts" || cmd === "/thoughts") {
                ...
                await this.reply(userId, contextToken, enabled
                    ? "已开启思考过程显示。\nThoughts enabled."
                    : "已关闭思考过程显示。\nThoughts disabled.");
                return true;
            }
    ...
    async handleTaskCommand(userId, contextToken, args, rawText) {
        const sub = args[0];
        ...
        if (sub === "new" || sub === "create") {
            ...
            try {
                const taskId = await this.acpSessions.createNewTask(userId, contextToken, description);
                await this.reply(userId, contextToken, `新任务 #${taskId} 已创建${description ? `：${description}` : ""}，已切换为当前任务。\nTask #${taskId} created${description ? `: ${description}` : ""}, now active.`);
            }
            catch (err) {
                await this.reply(userId, contextToken, `创建任务失败 / Failed to create task: ${String(err)}`);
            }
            return;
        }
        // /task list
        if (sub === "list" || sub === "ls") {
            const { tasks, activeTaskId } = this.acpSessions.listTasks(userId);
            if (tasks.length === 0) {
                await this.reply(userId, contextToken, "暂无活跃任务，发送消息会自动创建。\nNo active tasks. Send a message to start one.");
                return;
            }
        ...
        if (sub === "end" || sub === "kill" || sub === "stop" || sub === "close") {
            ...
            if (ended > 0) {
                ...
                await this.reply(userId, contextToken, `任务 #${ended} 已结束。${switchMsg}\nTask #${ended} ended. ${switchMsg}`);
            }
            else {
        ...
        if (sub && /^\d+$/.test(sub)) {
            ...
            if (ok) {
                await this.reply(userId, contextToken, `已切换到任务 #${taskId}，后续消息将发送到此任务。\nSwitched to task #${taskId}.`);
            }
            else {
                await this.reply(userId, contextToken, `任务 #${taskId} 不存在。使用 /task list 查看所有任务。\nTask #${taskId} not found. Use /task list.`);
            }
            return;
        }
        ...
        if (!sub) {
            const { tasks, activeTaskId } = this.acpSessions.listTasks(userId);
            if (tasks.length === 0) {
                await this.reply(userId, contextToken, "暂无活跃任务。发送 /task new 创建新任务。\nNo active tasks. Use /task new to create one.");
            }
            else {
                const active = tasks.find((t) => t.taskId === activeTaskId);
                const status = active?.processing ? "处理中/running" : "空闲/idle";
                await this.reply(userId, contextToken, `当前任务 / Current: #${activeTaskId} [${status}] ${active?.description ?? ""}\n共 ${tasks.length} 个任务 / ${tasks.length} total tasks\n\n/task list 查看全部 | /task new 新建`);
            }
            return;
        }
        // Unknown subcommand
        await this.reply(userId, contextToken, "用法 / Usage:\n/task new [描述] — 新建任务\n/task list — 查看任务\n/task <id> — 切换任务\n/task end [id] — 结束任务");
    }
    // -- API mode --
    async processApiMessage(msg) {
        ...
        try {
            ...
            if (firstTextItem?.text_item?.text) {
                const handled = await this.handleCommand(userId, contextToken, firstTextItem.text_item.text);
                if (handled)
                    return;
            }
            ...
            const response = await this.claude.chat(history, this.config.claude.systemPrompt);
            ...
            try {
                await this.sdk.sendText(userId, `Error: ${errorMessage}`, contextToken);
            }
            catch {
    ...
    async processAcpMessage(msg) {
        ...
        try {
            ...
            const acpBlocks = claudeBlocks.map((block) => {
                if (block.type === "text") {
    ...
    async sendProgressText(userId, contextToken, text) {
        if (this.debug) {
    ...
    async sendReply(userId, contextToken, text) {
        ...
        if (mediaPaths.length > 0) {
            const sent = this.sentFiles.get(userId) ?? new Set();
    ...
    extractFilePaths(text) {
        ...
        const exts = Object.keys(WeChatClaudeBridge.MEDIA_EXTENSIONS)
            ...
            .join("|");
        const pathPattern = new RegExp(`(\\/[^\\s\`"'<>|*?\\n]+\\.(?:${exts}))`, "gi");
     ...
     */
    async sendLocalFile(userId, contextToken, filePath) {
        ...
        try {
            ...
            logError("bridge", `Failed to send file ${filePath}: ${error instanceof Error ? error.message : error}`);
        }
    }
    async sendFileBuffer(userId, contextToken, data, fileName, mimeType) {
        ...
        try {
            ...
            logError("bridge", `Failed to send file ${fileName}: ${error instanceof Error ? error.message : error}`);
            await this.sdk.sendText(userId, `[File generated but failed to send: ${fileName}]`, contextToken);
        }
        finally {
            try {
                unlinkSync(tempPath);
    ...
    async sendImage(userId, contextToken, data, mimeType) {
        ...
        try {
            ...
            await this.sdk.sendText(userId, "[Image generated but failed to send]", contextToken);
        }
        finally {
            try {
                unlinkSync(tempPath);
