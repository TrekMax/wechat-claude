...
export class AcpSessionManager {
     ...
     */
    async createNewTask(userId, contextToken, description) {
        let group = this.users.get(userId);
        if (!group) {
            group = {
                userId,
    ...
    async createTask(userId, taskId, description, contextToken) {
    ...
    async processQueue(userId, task) {
        try {
            while (task.queue.length > 0 && !this.aborted) {
                ...
                task.client.updateCallbacks({
                    sendTyping: () => this.opts.sendTyping(userId, pending.contextToken),
                    onThoughtFlush: (text) => this.opts.onProgress(userId, pending.contextToken, text),
                    onToolProgress: (text) => this.opts.onProgress(userId, pending.contextToken, text),
                    onImageReceived: (data, mimeType) => this.opts.onImageReceived(userId, pending.contextToken, data, mimeType),
                    onFileReceived: (data, fileName, mimeType) => this.opts.onFileReceived(userId, pending.contextToken, data, fileName, mimeType),
                });
                ...
                try {
                    ...
                    this.opts.log(`[${userId}#${task.taskId}] Agent done (${result.stopReason}) in ${promptMs}ms, reply ${replyText.length} chars`);
                    ...
                    if (replyText.trim()) {
                        ...
                        await this.opts.onReply(userId, pending.contextToken, prefix + replyText);
                    }
                }
                catch (err) {
                    this.opts.log(`[${userId}#${task.taskId}] Agent prompt error: ${String(err)}`);
                    // Check if agent died
                    if (task.agentInfo.process.killed ||
                        task.agentInfo.process.exitCode !== null) {
                        this.opts.log(`[${userId}#${task.taskId}] Agent process died, removing task`);
                        const group = this.users.get(userId);
                        if (group) {
                    ...
                    try {
                        await this.opts.onReply(userId, pending.contextToken, `[Task #${task.taskId}] Agent error: ${String(err)}`);
                    }
                    catch {
                        // best effort
                    }
