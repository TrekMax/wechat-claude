...
const mode = flags.agent ? "acp" : "api";
const overrides = { mode, debug: flags.debug };
if (mode === "api") {
    const claudeOverrides = {};
    if (flags.model)
        claudeOverrides.model = flags.model;
    if (flags.maxTokens)
        claudeOverrides.maxTokens = parseInt(flags.maxTokens, 10);
    if (flags.systemPrompt)
        claudeOverrides.systemPrompt = flags.systemPrompt;
...
try {
    ...
    if (typeof qr.generate === "function") {
        renderQr = (url) => {
            qr.generate(url, { small: true }, (output) => {
                console.log(output);
            });
        };
    }
...
async function main() {
    ...
    if (mode === "api") {
        ...
        if (config.agent.showThoughts)
            console.log(`Thoughts: visible`);
    }
    if (config.debug)
    ...
    await bridge.start();
    console.log("[bridge] Ready! Send a message on WeChat to start chatting.");
}
main().catch((error) => {
    console.error("Fatal error:", error instanceof Error ? error.message : error);
    process.exit(1);
});
//# sourceMappingURL=wechat-claude.js.map